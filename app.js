...
app.patch('/api/tickets/:rid/status', requireAuth(), async (req, res) => {
  try {
    const { status, started_at, completed_lat, completed_lng, stuck_note, stuck_reason } = req.body || {};
    ...
    // เหตุผลติดขัด (รออะไหล่ / รอซัพ / อื่นๆ ที่ช่างพิมพ์เอง)
    // ส่งตรงไป FastAPI เพราะ larkService อาจไม่รู้จักฟิลด์นี้
    // ทำแบบ best-effort — ถ้าล้มเหลวไม่กระทบการเปลี่ยนสถานะที่สำเร็จไปแล้ว
    if (stuck_note !== undefined || stuck_reason !== undefined) {
      try {
        const axios = require('axios');
        const REPAIR_URL = process.env.REPAIR_API_URL || 'http://repair.mobile1234.site';
        const REPAIR_KEY = process.env.REPAIR_API_KEY || 'repair123';
        const payload = {};
        if (stuck_reason !== undefined) payload.stuck_reason = String(stuck_reason || '').slice(0, 60);
        if (stuck_note   !== undefined) payload.stuck_note   = String(stuck_note   || '').slice(0, 500);
        await axios.patch(`${REPAIR_URL}/api/tickets/${req.params.rid}`, payload, {
          headers: { 'X-API-Key': REPAIR_KEY, 'Content-Type':'application/json' }, timeout: 20000
        });
      } catch(e) {
        console.warn('[stuck_note] save failed:', e.response?.data?.detail || e.message);
      }
    }
